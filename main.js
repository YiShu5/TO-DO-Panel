...
const NETEASE_HISTORY_DATABASE = path.join(
  app.getPath('home'),
  'Library', 'Containers', 'com.netease.163music', 'Data', 'Documents',
  'storage', 'sqlite_storage.sqlite3'
);
const NETEASE_WEB_DATA_DIRECTORY = path.join(
  app.getPath('home'),
  'Library', 'Containers', 'com.netease.163music', 'Data', 'Documents',
  'storage', 'file_storage', 'webdata', 'file'
);
const NETEASE_TRACK_FILES = [
  path.join(NETEASE_WEB_DATA_DIRECTORY, 'playingList'),
  path.join(NETEASE_WEB_DATA_DIRECTORY, 'fmPlay'),
];
const NETEASE_DEBUG_HOST = '127.0.0.1';
const NETEASE_DEBUG_PORT = 9231;
...
function neteaseMusicRunning() {
  return new Promise((resolve) => {
    // 网易云可能带启动参数运行；只固定匹配可执行文件路径，不要求命令行到此结束。
  ...
  return new Promise((resolve) => {
    ...
    execFile(
      ...
      [
        ...
        NETEASE_MUSIC_APP,
        '--args',
        `--remote-debugging-address=${NETEASE_DEBUG_HOST}`,
        `--remote-debugging-port=${NETEASE_DEBUG_PORT}`,
      ],
...
let neteaseMusicIconCache = null;
let neteaseTrackFileCache = { signature: '', tracks: new Map() };

function requestLocalJson(port, requestPath, timeoutMs = 700) {
  return new Promise((resolve) => {
    const request = http.get({
      hostname: NETEASE_DEBUG_HOST,
      port,
      path: requestPath,
    }, (response) => {
...
async function getNeteaseDebuggerTarget() {
  const targets = await requestLocalJson(NETEASE_DEBUG_PORT, '/json');
  if (!Array.isArray(targets)) return null;
  return targets.find((target) => target
    && target.type === 'page'
    && /^orpheus:\/\/orpheus\//.test(String(target.url || ''))
    && String(target.webSocketDebuggerUrl || '').startsWith(`ws://${NETEASE_DEBUG_HOST}:${NETEASE_DEBUG_PORT}/`)) || null;
}

async function evaluateNeteasePage(expression) {
  const target = await getNeteaseDebuggerTarget();
  ...
  return new Promise((resolve) => {
    ...
    socket.on('message', (data) => {
      try {
        ...
        finish(message.result && message.result.result && message.result.result.value || null);
      } catch (error) {
        finish(null);
      }
    });
...
const NETEASE_STATUS_EXPRESSION = `(() => {
  const root = document.getElementById('page_pc_mini_bar');
  if (!root) return null;
  let songId = '';
  try {
    const log = JSON.parse(root.getAttribute('data-log') || '{}');
    songId = String(log && log.params && log.params.s_cid || '');
  } catch (error) {}
  const title = String(root.querySelector('.vinly-title, .main-title .title')?.textContent || '').trim();
  const artist = String(root.querySelector('.author')?.textContent || '').trim();
  return {
    id: songId,
    title,
    artist,
    playing: Boolean(root.querySelector('.cmd-icon-pause')),
  };
})()`;

async function readNeteaseLiveStatus() {
  const value = await evaluateNeteasePage(NETEASE_STATUS_EXPRESSION);
  return value && typeof value === 'object' && value.title ? value : null;
}

function readNeteaseTrackFiles(trackId) {
  const id = String(trackId || '');
  if (!id) return null;
  const signature = NETEASE_TRACK_FILES.map((filePath) => {
    try {
  ...
  if (signature !== neteaseTrackFileCache.signature) {
    const tracks = new Map();
    for (const filePath of NETEASE_TRACK_FILES) {
      let payload;
      try { payload = JSON.parse(fs.readFileSync(filePath, 'utf8')); } catch (error) { continue; }
      const rows = [
        ...(Array.isArray(payload.list) ? payload.list : []),
        ...(Array.isArray(payload.queue) ? payload.queue : []),
      ];
      for (const row of rows) {
        const track = row && row.track || row;
        const metadata = parseNeteaseHistoryTrack(track);
        if (metadata && metadata.id) tracks.set(metadata.id, metadata);
      }
    }
    neteaseTrackFileCache = { signature, tracks };
  }
  return neteaseTrackFileCache.tracks.get(id) || null;
}

function readNeteaseHistoryTrack(trackId = '') {
  if (!fs.existsSync(NETEASE_HISTORY_DATABASE)) return Promise.resolve(null);
  const id = String(trackId || '');
  const where = /^\d+$/.test(id) ? ` WHERE id = '${id}'` : '';
  return new Promise((resolve) => {
    execFile(
      '/usr/bin/sqlite3',
      [
        ...
        '-cmd',
        '.timeout 250',
        NETEASE_HISTORY_DATABASE,
        `SELECT jsonStr FROM historyTracks${where} ORDER BY playtime DESC LIMIT 1;`,
      ],
      { timeout: 1200, maxBuffer: 512 * 1024 },
      (error, stdout) => {
        if (error) return resolve(null);
        resolve(parseNeteaseHistoryTrack(String(stdout || '').trim()));
      }
    );
  });
}

async function readNeteaseMusicIcon() {
  if (!neteaseMusicIconCache) neteaseMusicIconCache = await readSystemAppIconNow(NETEASE_MUSIC_APP);
  return neteaseMusicIconCache;
}

async function sendNeteaseDebuggerControl(action) {
  if (!['play', 'pause', 'next', 'previous'].includes(action)) return { ok: false, error: 'invalid_action' };
  const expression = `(() => {
    const root = document.getElementById('page_pc_mini_bar');
    if (!root) return { ok: false, error: 'player_unavailable' };
    const action = ${JSON.stringify(action)};
    const playing = Boolean(root.querySelector('.cmd-icon-pause'));
    if ((action === 'play' && playing) || (action === 'pause' && !playing)) {
      return { ok: true, playing };
    }
    const selectors = {
      play: '.cmd-icon-play',
      pause: '.cmd-icon-pause',
      next: '.cmd-icon-next',
      previous: '.cmd-icon-pre, .cmd-icon-prev, .cmd-icon-previous',
    };
    const control = root.querySelector(selectors[action]);
    if (!control) return { ok: false, error: 'control_unavailable' };
    control.click();
    return { ok: true, playing: action === 'pause' ? false : true };
  })()`;
  const result = await evaluateNeteasePage(expression);
  return result && result.ok ? result : { ok: false, error: result && result.error || 'debugger_unavailable' };
}

const NETEASE_QUIT_JXA = `
function findMenuItem(items, acceptedNames) {
  for (let index = 0; index < items.length; index += 1) {
    let title = '';
    try { title = String(items[index].name() || ''); } catch (error) {}
    if (acceptedNames.indexOf(title) >= 0) return items[index];
  }
  return null;
}
...
function run() {
  ...
  const processes = systemEvents.applicationProcesses.whose({ bundleIdentifier: 'com.netease.163music' })();
  if (!processes.length) return 'missing';
  const process = processes[0];
  try {
    const appMenu = findMenuItem(process.menuBars[0].menuBarItems(), ['网易云音乐', 'NeteaseMusic']);
    const quitItem = appMenu && findMenuItem(appMenu.menus[0].menuItems(), [
      '退出 网易云音乐', '退出网易云音乐', 'Quit NeteaseMusic',
    ]);
    if (!quitItem) return 'menu-unavailable';
    quitItem.click();
    return 'ok';
  } catch (error) {
...
async function restartNeteaseMusicForSync() {
  if (await getNeteaseDebuggerTarget()) return true;
  if (!await neteaseMusicRunning()) return launchNeteaseMusic();
  if (!systemPreferences.isTrustedAccessibilityClient(false)) return false;
  let quitResult;
  try { quitResult = await runJxa(NETEASE_QUIT_JXA, []); } catch (error) { return false; }
  if (quitResult !== 'ok') return false;
  for (let attempt = 0; attempt < 30; attempt += 1) {
    if (!await neteaseMusicRunning()) break;
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
  if (await neteaseMusicRunning()) return false;
  if (!await launchNeteaseMusic()) return false;
...
function run(argv) {
  ...
  try {
    const menuBarItems = process.menuBars[0].menuBarItems();
    const controlMenuItem = findMenuItem(menuBarItems, ['控制', 'Controls']);
    if (controlMenuItem) {
      const items = controlMenuItem.menus[0].menuItems();
      const playItem = findMenuItem(items, ['播放', 'Play']);
      const pauseItem = findMenuItem(items, ['暂停', '暫停', 'Pause']);
      let target = null;
      if (action === 'play') {
        if (pauseItem) return 'already';
        target = playItem;
      } else if (action === 'pause') {
        if (playItem) return 'already';
        target = pauseItem;
      } else if (action === 'next') {
        target = findMenuItem(items, ['下一个', '下一個', '下一首', 'Next']);
      } else if (action === 'previous') {
        target = findMenuItem(items, ['上一个', '上一個', '上一首', 'Previous']);
      }
      if (target) {
        target.click();
        return 'ok';
...
async function sendNeteaseControl(action) {
  ...
  const debuggerResult = await sendNeteaseDebuggerControl(action);
  ...
  try {
    ...
    return ['ok', 'already'].includes(result)
      ? { ok: true }
      : { ok: false, error: 'netease_control_failed' };
  } catch (error) {
...
ipcMain.handle('music:status', async () => {
  ...
  const live = running ? await readNeteaseLiveStatus() : null;
  const track = live
    ? readNeteaseTrackFiles(live.id) || await readNeteaseHistoryTrack(live.id)
    : running ? await readNeteaseHistoryTrack() : null;
  if (live && typeof live.playing === 'boolean') neteaseMusicPlaying = live.playing;
  return {
    installed,
    running,
    sessionActive: running,
    playing: running && neteaseMusicPlaying,
    title: live && live.title || track && track.title || '',
    artist: live && live.artist || track && track.artist || '',
    artwork: track && track.artwork || '',
    synchronized: Boolean(live),
    icon: installed ? await readNeteaseMusicIcon() : null,
  };
});

ipcMain.handle('music:control', async (event, action) => {
  if (!fs.existsSync(NETEASE_MUSIC_APP)) return { ok: false, error: 'not_installed' };
  // 操作网易云的瞬间 macOS 可能短暂转移焦点；这属于卡片内部交互，不应触发灵动岛收起。
  transientSystemInteractionRequests += 1;
  try {
    if (await neteaseMusicRunning() && !await getNeteaseDebuggerTarget()) {
      await restartNeteaseMusicForSync();
    }
    const result = await controlNeteaseMusic(action, {
      isRunning: neteaseMusicRunning,
      launch: launchNeteaseMusic,
      sendShortcut: sendNeteaseControl,
    }, neteaseMusicPlaying);
