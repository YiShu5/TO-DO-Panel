...
async function main() {
  ...
  try {
    ...
    const musicCardInteraction = await window.webContents.executeJavaScript(`
      (async () => {
        const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
        const waitForClass = async (name) => {
          const deadline = performance.now() + 5000;
        ...
        await sleep(30);
        document.getElementById('music-title').dispatchEvent(new MouseEvent('click', {
          bubbles: true,
          cancelable: true,
        }));
        await sleep(30);
        const backgroundKeptOpen = document.getElementById('app').classList.contains('expanded');
        document.getElementById('music-play-toggle').dispatchEvent(new MouseEvent('click', {
          bubbles: true,
          cancelable: true,
        }));
        await sleep(30);
        const controlKeptOpen = document.getElementById('app').classList.contains('expanded');
        notch.click();
        const cleanedUp = await waitForClass('collapsed');
        return { opened, backgroundKeptOpen, controlKeptOpen, cleanedUp };
      })()
    `);
    assert.deepEqual(musicCardInteraction, {
      opened: true,
      backgroundKeptOpen: true,
      controlKeptOpen: true,
      cleanedUp: true,
    }, '音乐卡片空白区与控制键都必须留在展开的灵动岛内');
