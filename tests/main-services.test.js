...
test('Netease Music history rows expose synchronized song metadata', () => {
  const metadata = parseNeteaseHistoryTrack(JSON.stringify({
    id: 229336,
    name: '一个像夏天一个像秋天',
    artists: [{ name: '范玮琪' }, { name: '合唱歌手' }],
    album: { picUrl: 'http://p4.music.126.net/cover.jpg' },
  }));
  assert.deepEqual(metadata, {
    id: '229336',
    title: '一个像夏天一个像秋天',
    artist: '范玮琪 / 合唱歌手',
    artwork: 'https://p4.music.126.net/cover.jpg?param=256y256',
  });
  assert.equal(parseNeteaseHistoryTrack('{broken'), null);
  assert.equal(parseNeteaseHistoryTrack({ artists: [] }), null);
});
...
test('running Netease Music uses its own controls because native media status can stay empty', async () => {
  const events = [];
  const result = await controlNeteaseMusic('play', {
...
test('Netease Music fallback shortcuts match the installed macOS client menu', () => {
  // 网易云音乐 3.1.11 的控制菜单声明：Space、Cmd+Right、Cmd+Left。
  assert.deepEqual(neteaseShortcutSpec('play'), { keyCode: 49, command: false, dismissOverlays: true });
  assert.deepEqual(neteaseShortcutSpec('pause'), { keyCode: 49, command: false, dismissOverlays: true });
  assert.deepEqual(neteaseShortcutSpec('next'), { keyCode: 124, command: true, dismissOverlays: true });
  assert.deepEqual(neteaseShortcutSpec('previous'), { keyCode: 123, command: true, dismissOverlays: true });
  assert.notDeepEqual(neteaseShortcutSpec('play'), neteaseShortcutSpec('next'));
  assert.equal(neteaseShortcutSpec('invalid'), null);
});
