...
test('Netease Music card renders synchronized title, artist, controls and ambient motion without artwork', () => {
  ...
  assert.match(html, /id="music-title"/);
  assert.match(html, /id="music-artist"/);
  assert.doesNotMatch(workspaceJs, /status\.artwork \|\| status\.icon/);
  assert.match(workspaceJs, /classList\.toggle\('music-playing'/);
  assert.match(workspaceJs, /\[180, 650, 1400\]/);
  assert.match(html, /img-src 'self' data:/);
  assert.doesNotMatch(stylesCss, /url\('assets\/generated\/music-bg-/);
  assert.match(stylesCss, /music-ambient-drift/);
  assert.match(stylesCss, /music-playing \.music-ambient/);
});
