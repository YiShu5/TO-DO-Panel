(function initWorkspace() {
  ...
  async function refreshMusicStatus() {
    ...
    const hasTrack = Boolean(status && status.title);
    if (musicTitle) musicTitle.textContent = hasTrack ? status.title : status && status.installed ? '网易云音乐' : '未安装网易云音乐';
    if (musicArtist) musicArtist.textContent = hasTrack
      ? status.artist || '未知歌手'
      : status && status.installed ? '轻触播放，歌曲信息会自动同步' : '请先安装网易云音乐';
    if (musicStatus) musicStatus.textContent = status && status.running
      ? status.synchronized === false ? '轻触控制以同步' : musicPlaying ? '正在播放' : hasTrack ? '已暂停' : '网易云音乐'
      : status && status.installed ? '尚未运行' : '需要本地客户端';
    if (homeMusic) homeMusic.setAttribute('aria-label', hasTrack
      ? `${status.title}${status.artist ? `，${status.artist}` : ''}`
      : '网易云音乐播放器');
  }

  homeMusic?.addEventListener('click', async (event) => {
    if (event.target.closest('[data-widget-size-cycle]')) return;
    // 音乐卡片属于展开面板内部：点击留在岛内，只有三枚控制键执行播放操作。
    event.stopPropagation();
    const control = event.target.closest('[data-music-action]');
    if (!control || !window.notchAPI) return;
    ...
    if (!result || !result.ok) {
      const needsSession = result && ['no_active_session', 'netease_session_inactive'].includes(result.error);
    ...
    [180, 650, 1400].forEach((delay) => setTimeout(refreshMusicStatus, delay));
  });
  ...
  setInterval(() => {
    if (workspaceExpanded && workspaceTab === 'home' && window.NotchHome?.isVisible?.('music') !== false) {
      refreshMusicStatus();
    }
  }, 1800);
