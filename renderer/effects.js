...
(function bootstrapPanelEffects() {
  ...
  const musicEffect = createWebglEffect(
    ...
    {
      setup({ gl, uniform }) {
        gl.uniform1f(uniform('uSpeed'), 0.16);
        gl.uniform1i(uniform('uColorCount'), 3);
        gl.uniform3f(uniform('uColors[0]'), 0.38, 0.07, 0.14);
        gl.uniform3f(uniform('uColors[1]'), 0.62, 0.14, 0.22);
        gl.uniform3f(uniform('uColors[2]'), 0.16, 0.09, 0.29);
        gl.uniform1i(uniform('uTransparent'), 1);
        gl.uniform1f(uniform('uScale'), 1.35);
        gl.uniform1f(uniform('uFrequency'), 0.72);
        gl.uniform1f(uniform('uWarpStrength'), 0.38);
        gl.uniform1f(uniform('uMouseInfluence'), 0.2);
        gl.uniform1f(uniform('uParallax'), 0.12);
        gl.uniform1f(uniform('uNoise'), 0.04);
        gl.uniform1i(uniform('uIterations'), 1);
        gl.uniform1f(uniform('uIntensity'), 0.58);
        gl.uniform1f(uniform('uBandWidth'), 9);
      },
